...
func DeleteinboundconnectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2021-01-01/opensearch/cc/inboundConnection/%s", cfg.BaseURL, ConnectionId)
		...
		var result models.DeleteInboundConnectionResponse
...
func CreateDeleteinboundconnectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_2021-01-01_opensearch_cc_inboundConnection_ConnectionId",
		...
		mcp.WithString("ConnectionId", mcp.Required(), mcp.Description("The ID of the inbound connection to permanently delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteinboundconnectionHandler(cfg),
	}
}
